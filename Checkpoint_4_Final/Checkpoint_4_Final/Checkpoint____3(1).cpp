...
int ed = 5;//determines the incoming speed of the eney
int lvl = 1; char lvlarray[20];//used to indicate level and show the level

int score =0; char scorearray[20]; int savescore;//for storing the score and show them
int tc;
int a1;//x axis position for enemies;
int b1;//yxxis position for fireball;
...
int poogy[2], bck, nm, bck2,dragon;//poggy stores images andbck andbck are backgrounds, dragon for dragon pic, which was no impimented
int width = 1000; //for width
...
void EnemyScene()//controls the enemy structures which also implements a timer
{
	...
	enm1.enemyxpos -= ed;enm2.enemyxpos -= ed;enm3.enemyxpos -= ed;enm4.enemyxpos -= ed;enm5.enemyxpos -= ed;
}
